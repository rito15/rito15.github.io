...
# 개념
---
 - 맵과 크기가 동일한 RGBA(0, 0, 0, a)의 텍스쳐를 이용해 지정한 유닛들의 시야를 표현한다.
...
<정의>
 ...
 [4] InRangeList 내 모든 타일들에 대해 각각 순회하며 검사한다.
   ...
   [4-2] UC > UO이면서 (nDot(UC, UO) > 0.9)인 O가 존재하지 않는 경우 C <- T
...
## 잡 시스템 적용 결과
- 주요 병목이 되는 주기적 장애물 검사 알고리즘을 IJobParallelFor를 이용해 돌려보았지만, 유의미한 성능 향상이 보이지 않는다. 나중에 잡을 더 깊게 익히면 다시 성능 최적화에 도전해봐야 할 듯하다.
...
# Download
---
- [Fog of War.zip](https://github.com/rito15/Images/files/5868054/Fog.of.War.zip)