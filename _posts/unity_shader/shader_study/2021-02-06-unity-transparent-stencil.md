...
DirectX에서는 **알파 테스트(Alpha Test)**, OpenGL에서는 **컷아웃(Cutout)**이라고 부른다.

<br>

## **특징**
 - 풀, 나뭇잎 등의 환경 표현에 주로 사용된다.
 - 지정한 `_Cutoff` 값보다 큰 알파값을 가지는 부분만 그려주고, 작은 값을 가지는 부분은 잘라낸다.
...
반대로, **ZTest**의 결과가 거짓이라 픽셀을 그리지 못했다면 **"ZTest에 실패했다"**라고 하며
