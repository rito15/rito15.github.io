...
`.NET` 환경에서 `CLR` 가상머신이 런타임에 `JIT` 컴파일을 통해 입맛대로 맛보고 즐길 수 있도록 만들어진 `CIL`을 통해 만들어진 바이너리 내부에 있는 프로그램 코드 부분을 `CIL Code`라고 한다.
...
`.NET` 환경에서 `JIT` 컴파일이 가능하도록 만들어진 바이너리이면 그냥 뭐.. `.NET Binary Code`라고 불러야 할 것 같다.
...
![image](https://user-images.githubusercontent.com/42164422/120902267-4a500500-c67a-11eb-999b-1ba314ea399d.png)
...
- 각각 `JVM`과 `CLR`이 런타임에 `JIT` 컴파일을 통해 네이티브 코드를 생성하는 과정을 거쳐야 하므로 느리다.
